...
Dependencies: CONFIG, COLORS, EMOJI, BRC.util.color, BRC.mpr.que, BRC.mpr.que_optmore
--]]
...
local function format_delta(delta)
  if delta > 0 then
    return BRC.util.color(COLORS.green, "+" .. delta)
  elseif delta < 0 then
    return BRC.util.color(COLORS.red, delta)
  else
    return BRC.util.color(COLORS.darkgrey, "+0")
  end
end
