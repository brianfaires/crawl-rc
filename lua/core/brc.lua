...
-- Hotkey functions
local function hotkey_is_live()
  return _hotkey_action and you.turns() <= hotkey_timeout
end

local function c_message_autoequip(it)
  if hotkey_is_live() or not (it.is_weapon or BRC.is.armour(it) or BRC.is.jewellery(it)) then return end
  local TURNS = 1
  local name = it.name():gsub(" {.*}", "")

  BRC.mpr.que("Press " .. BRC.Config.hotkey.key.name .. " to equip " .. name .. ".", BRC.COLOR.darkgrey)

  BRC.set_hotkey(function()
...
function BRC.set_hotkey(func, turns_available)
  _hotkey_action = func
  hotkey_timeout = you.turns() + (turns_available or 1)
end

function macro_brc_do_hotkey()
  if hotkey_is_live() then
    _hotkey_action()
  else
...
function BRC.init()
  ...
  BRC.set.macro("\\{" .. BRC.Config.hotkey.key.keycode .. "}", "macro_brc_do_hotkey")
...
function BRC.c_assign_invletter(it)
  if not BRC.active then return end
  if BRC.Config.hotkey.autoequip then c_message_autoequip(it) end
