...
local function update_pack_mutes()
  ...
  for _, v in ipairs(monsters_to_mute) do
    ...
    BRC.mpr.blue(string.format("  Muted: %s", v))
  end
  monsters_to_mute = {}

  -- Remove expired mutes
  for i, v in ipairs(FM_PATTERNS) do
    if v.is_pack and last_fm_turn[i] ~= -1 and you.turns() >= last_fm_turn[i] + BRC.Config.pack_timeout then
      last_fm_turn[i] = -1
      active_alert[i] = false -- Set to false and let the main logic decide if it should reactivate it.
      BRC.mpr.blue(string.format("  Unmuted: %s", v.pattern))
    end
  end
end
...
function f_alert_monsters.init()
  ...
  -- Convert table patterns to strings, and init
  for i, v in ipairs(FM_PATTERNS) do
...
function f_alert_monsters.c_message(text, channel)
  ...
  for i, v in ipairs(FM_PATTERNS) do
    if v.is_pack and text:find(v.pattern) and last_fm_turn[i] == -1 then
      last_fm_turn[i] = you.turns()
      monsters_to_mute[#monsters_to_mute + 1] = v.pattern
      BRC.mpr.blue(string.format("To mute: %s", v.pattern))
    end
  end
end
