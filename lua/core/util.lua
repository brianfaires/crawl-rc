...
local function tostr_list(var, pretty, indents)
  ...
  for _, v in ipairs(var) do
    tokens[#tokens + 1] = cap_lines(BRC.util.tostring(v, pretty, indents + 1))
  end
...
local function tostr_map(var, pretty, indents)
  ...
  if pretty then
    ...
    for i = 1, #keys do
      local v = cap_lines(BRC.util.tostring(var[keys[i]], true, indents + 1))
      if v then
    ...
    for k, v in pairs(var) do
      tokens[#tokens + 1] = '["' .. k .. '"] = ' .. BRC.util.tostring(v, pretty, indents + 1)
    end
  end
...
function BRC.util.tostring(var, pretty, _indents)
  ...
  if BRC.Data.Config.omit_pointers and (var_type == "function" or var_type == "userdata") then
    return nil
  end
