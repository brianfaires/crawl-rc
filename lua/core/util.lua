...
function init_util() delayed_mpr_queue = {} end
...
function with_color(color, text) return string.format("<%s>%s</%s>", color, text, color) end

--- Key modifiers ---
function control_key(c) return string.char(string.byte(c) - string.byte("a") + 1) end
...
function get_mut(mutation, include_all) return you.get_base_mutation_level(mutation, true, include_all, include_all) end

function get_skill_with_item(it)
  if is_magic_staff(it) then return math.max(get_skill(get_staff_school(it)), get_skill("Staves")) end
...
function have_shield() return is_shield(items.equipped_at("offhand")) end

function have_weapon() return items.equipped_at("weapon") ~= nil end

function have_zero_stat() return you.strength() <= 0 or you.dexterity() <= 0 or you.intelligence() <= 0 end

function in_hell() return util.contains(ALL_HELL_BRANCHES, you.branch()) end

function is_amulet(it) return it and it.name("base") == "amulet" end
...
function is_aux_armour(it) return is_armour(it) and not (is_body_armour(it) or is_shield(it)) end

function is_body_armour(it) return it and it.subtype() == "body" end

function is_jewellery(it) return it and it.class(true) == "jewellery" end

function is_magic_staff(it) return it and it.class and it.class(true) == "magical staff" end
...
function is_mutation_immune() return util.contains(ALL_UNDEAD_RACES, you.race()) end

function is_ring(it) return it and it.name("base") == "ring" end

function is_scarf(it) return it and it.class(true) == "armour" and it.subtype() == "scarf" end

function is_shield(it) return it and it.is_shield() end
...
function is_orb(it) return it and it.class(true) == "armour" and it.subtype() == "offhand" and not it.is_shield() end

function is_polearm(it) return it and it.weap_skill:find("Polearms", 1, true) end
...
function dump_chk_lua_save(char_dump) dump_text(serialize_chk_lua_save(), char_dump) end

function dump_inventory(char_dump, include_item_info) dump_text(serialize_inventory(include_item_info), char_dump) end
