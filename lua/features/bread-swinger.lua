...
f_bread_swinger.Config = {
  ...
  alert_for_slowest_weapon = true, -- Alert each time you find a weapon with largest delay yet.
...
local function is_plant(x, y)
  local mon = monster.get_monster_at(x, y)
  return mon and mon.is_stationary() -- Close enough. We know you.feel_safe() == true
end
...
local function is_good_dir_swing(x, y)
  ...
  if Config.allow_plant_damage then
    return weapon.is_ranged or not is_solid(x,y)
  elseif weapon.is_ranged then
    -- Confirm no plants in straight line
    for i = 1, you.los() do
      local cur_x = i * x
      local cur_y = i * y
      if is_plant(cur_x, cur_y) then return false end
      if is_solid(cur_x, cur_y) then break end
    end
  elseif weapon.weap_skill:contains("Axes") then
    -- Confirm no plants in adjacent squares
    for cur_x = -1, 1 do
      for cur_y = -1, 1 do
        if is_plant(cur_x, cur_y) then return false end
      end
