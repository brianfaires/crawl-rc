...
function BRC.util.tostring(value, format_for_display, indent_count)
  local var_type = type(value)
  if var_type == "string" then
    local s = '"' .. value:gsub('"', "") .. '"'
    if not format_for_display then return s end
  ...
  elseif var_type == "table" then
    ...
    if BRC.is.list(value) then
      ...
      for _, v in ipairs(value) do
        tokens[#tokens + 1] = BRC.util.tostring(v, format_for_display, indent_count + 1)
      end
      if #tokens < 4 and not util.exists(value, function(t) return type(t) == "table" end) then
    ...
    elseif BRC.is.map(value) then
      ...
      for k, v in pairs(value) do
        local form = '["%s"] = %s'
        if format_for_display then
          if type(v) ~= "table" then form = "%s = %s" end
          contains_table = contains_table or type(v) == "table"
        end
        tokens[#tokens + 1] = string.format(form, k, BRC.util.tostring(v, format_for_display, indent_count + 1))
      end
      if format_for_display and #tokens <= 2 and not contains_table then
        return string.format("{ %s }", table.concat(tokens, ", "))
      else
        return "{\n" .. child_indent .. table.concat(tokens, list_separator) .. "\n" .. indent .. "}"
      end
